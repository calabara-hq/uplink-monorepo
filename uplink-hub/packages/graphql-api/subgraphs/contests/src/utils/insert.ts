...
export const insertUniqueTokens = async (spaceId: number, tokens: ExtractedTokenCache, tx: any): Promise<InsertedTokenCache> => {
    ...
    for (const extractedHash of Object.keys(tokens)) {
        if (!insertedTokens[extractedHash]) {
            ...
            const remoteToken = await tx.select({ id: schema.tokens.id }).from(schema.tokens).where(sqlOps.eq(schema.tokens.tokenHash, extractedHash))
            ...
            if (!remoteToken[0]) {
                ...
                await insertSpaceToken(spaceId, parseInt(insertedToken.insertId), tx); // update space token mapping
            } else {
                // have already inserted remotely
                insertedTokens[extractedHash] = { dbTokenId: parseInt(remoteToken[0].id) }; // add to cache
                await insertSpaceToken(spaceId, parseInt(remoteToken[0].id), tx); // update space token mapping
            }
        }
    }
...
const handleRewards = async (contestId: number, rewards: PreparedRewards, rewardType: 'submitter' | 'voter', tx: any) => {
    ...
    for (const reward of rewards) {
        ...
        const newTokenReward: schema.dbNewTokenRewardType = {
            ...
            amount: 'amount' in reward.value ? reward.value.amount : null,
